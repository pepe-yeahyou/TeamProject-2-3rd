...
function Detail() {
    ...
    // 💡 [추가] 기간 만료 체크 로직
    // project.endDate가 있고, 오늘 날짜가 종료일보다 늦으면 만료
    ...
    // 💡 [수정] 권한 변수: 기간이 만료되었으면(isExpired) Task 변경 권한을 false로 강제함
    ...
    const handleTaskStatusToggle = async (taskId, currentStatus) => {
        ...
        try {
            await axios.post(`${API_BASE_URL}/${projectId}/task/${taskId}?isCompleted=${isCompleted}`);
    ...
    // 💡 [수정] 상태 결정 로직 (완료 > 기간만료 > 진행중 순위)
    let projectStatus = calculatedProgress === 100 ? '완료' : (isExpired ? '기간만료' : '진행중');
    ...
    return (
        <div className="detail-page"> 
            <div className="main-content">
                <div className="detail-card title-section">
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                        <h2>
                            {project.title} 
                            {/* 💡 클래스명이 status-기간만료 가 되도록 템플릿 리터럴 적용 */}
                    ...
                    {/* 💡 [추가] 기간 표시 영역 */}
                    <div className="project-period" style={{ marginTop: '10px', fontSize: '0.9em', color: '#888' }}>
                ...
                <div className="detail-card progress-section">
                    ...
                    <div className="progress-bar-container">
                        <div className="progress-bar" style={{ width: `${calculatedProgress}%` }}></div>
                    </div>
                </div>
            ...
            <div className="sidebar">
                ...
                {/* 💡 [수정] 채팅 섹션: '진행중'일 때만 활성화 (기간만료나 완료 시 비활성화) */}
                {projectStatus === '진행중' ? (
                    <div className="detail-card chat-section">
                        ...
                        <Chat 
                            ...
                            isChatEnabled={hasTaskPermission} // 기간만료면 false가 됨
                        />
                    </div>
                ) : (
                    <div className="detail-card completion-message">
                        <h4>{projectStatus === '완료' ? '해당 프로젝트는 완료되었습니다.' : '기간이 만료되어 채팅이 불가합니다.'}</h4>
                    </div>
                )}
            </div>
        </div>
